...
function Csing(){
  if(status != 0){
    ...
    if(!nskp){con.onclick = function(){skip = true}};
    con.style.cursor = 'default';
      ...
      case '%':
        if(vars.hasOwnProperty(vvn[2])){
          if(!isNaN(vars[vvn[2]])){
            ...
            console.log('mod vars error.');
          }
        }else if(vvn[2].match(/^\d*.?\d+$/)){
          if(!isNaN(vars[vns])){vars[vns] %= Number(vvn[2])}
          else{console.log('mod vars error.')}
        }else{
          console.log('mod vars error.');
        }
      break;
      default:
        if(vvn[2] == 'rnd'){vars[vns] = Math.floor(232792560 * Math.random())}
...
function condition(n, c, v){//c:M|L|E|NE
  var vs;
...
var tags = ['a', 'div', 'span', 'p', 'img', 'input', 'var', 'if', 'spd', 'next', 'keep', 'skip', 'bg', 'color', 'lh', 'audio', 'nskp', 'reset', 'ev'];
var tf = {next: snext, keep: skeep, skip: sskip, spd: sspd, color: scolor, lh: slh, var: svar, if: sif, bg: sbg, img: simg, input: sinput, audio: saudio, a: stag, span: stag, div: stag, p: stag, nskp: snskp, ev: sev};
function sev(str, tag, d){
  var tmp;
  if(tmp = tag.match(/^=(['"])(.*?[^\\])\1\s*\/?$/)){
    eval(tmp[2]);
  }
  return [str.replace(/^<ev.*?>/, ''), d];
}
var nextA = false;
var nextP = null;
var ant;
...
var nskp = false;
function snskp(str, t, d){
  nskp = true;
  return [str.replace(/^<nskp.*?>/, ''), d];
}
var spd = [dspd];
...
function sbg(str, tag, d){
  ...
  if(bg = tag.match(/^=(['"])(.*?)\1.*?$/)){
    ...
    con.style.backgroundSize = w ? (h ? w + " " + h: w + ' auto'): (h ? 'auto ' + h: 'auto');
  }
...
function sinput(str, tag, dom){
  ...
  if(inp = tag.match(/^[^>]*\sn=(['"])(.*?)\1/)){
    ...
    if(inpv = tag.match(/^[^>]*\sv=(['"])(.*?)\1/)){
      ...
      if(inpv[1] == 'rnd'){
        inpt.setAttribute('value', Math.floor(232792560 * Math.random())); 
...
function show(st, DOM){
  var tag, res, tadj;
  tadj = performance ? performance.now(): (new Date().getTime())%100000;
  if(st.length > 0){
    st = st.toString();
    res = [st, DOM];
    while(tag = res[0].match(RegExp('^<(' + tags.join('|') + ')(\\s+[^\\s]+.*?|=.*?|\\/?)>'))){
      if(tag[1] == 'reset'){return reset(1)}
    ...
    Csing();
    if(skip || (spd[spd.length - 1] == 0)){
      display(res[0], res[1]);
    }else{
      window.setTimeout(function(){display(res[0], res[1])}, tadj - (performance ? performance.now(): (new Date().getTime()) % 100000) + spd[spd.length - 1]);
    }
  }else{
    pointerN(nextP);
    Cwait();
    if(sel && nextA){ant = window.setTimeout(nextC, spd[spd.length - 1])}
    else if(nextA){nextC()}
  }
}
...
function initial(){
  ...
  line.style.width = (cw - lpx * 2) + 'px';
  line.style.height = (ch - 2 - lpy * 2) + 'px';
  ...
  line.style.padding = lpy + 'px ' + lpx + 'px';
  ...
  hint.style.backgroundColor = 'rgba(255,255,255,0.4)';
  hint.style.zIndex = '19999';
  hint.style.display = 'none';
  hint.style.position = 'absolute';
  hint.onmousedown = function(e){e.preventDefault()}
